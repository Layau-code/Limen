...
func TestGatewayTargetPreservesPricing(t *testing.T) {
	...
	if target.Provider != source.Provider || target.UpstreamModel != source.UpstreamModel || target.EndpointID != source.EndpointID || target.Pricing == nil || target.Pricing.InputPerMillionNanoUSD != 11 {
