...
- `limen demo` 现在同时演示 `model=auto` 能力契约选模、被排除目标的稳定原因码、Fallback 和配置草稿 Replay，输出安全的逻辑模型选择摘要。
...
- 真实二进制冒烟新增 `SIGTERM` 关闭检查，确认服务先撤销 readiness 再退出。
- README 和运维文档补充 `LIMEN_RUN_STORE`、`LIMEN_HEALTH_URL` 的使用边界，降低首次部署和 Run 演示的配置歧义。
- Anthropic SSE 现在会传播上游 `error` 事件，并拒绝在缺少 `message_stop` 时伪造 `[DONE]`，避免截断响应被客户端误判为成功。
- Executor 现在会关闭 Provider 在返回错误时意外附带的非空响应体，避免异常适配器泄漏上游连接。
- 不支持的 Chat 消息角色错误不再回显客户端提供的角色字符串，保持错误响应稳定且不带用户输入。
- 修复租约过期后已知结算任务无法恢复 Run 的问题：已知费用不会被降级为未知，也不会重复减少 `in_flight`，处理后按预算、截止时间和完成标记恢复 Run。
- PostgreSQL Key Store 返回的跨租户 Principal 现在会在 HTTP 入口被拒绝；每个进程只服务 `LIMEN_TENANT_ID`，避免其他租户进入共享 Router、配置、熔断状态或 Provider 凭据路径。
- 配置版本切换现在只保留当前目标和仍在执行旧 Run 的历史熔断器；执行引用释放后会在下一次切换时回收无引用状态，避免长期发布导致进程内存无界增长。
- 熔断器不再固化首次创建时的阈值和冷却参数；Decision 与 Executor 都使用本次 Run 固定的路由策略，避免配置发布后新旧策略互相污染。
- 普通响应和 SSE 转发现在传播响应体读取/写入错误，并将已开始但中断的响应标记为 `partial` 结算；不会在提交正文后执行 Fallback 或伪造完整用量。
...
- 服务启动现在先绑定监听 socket，再设置 readiness 并调用 `Serve`，避免端口绑定失败时出现虚假的就绪状态。
- PostgreSQL 迁移现在在版本检查前获取事务级 advisory lock，并新增并发启动集成测试，避免多实例同时执行 DDL。
