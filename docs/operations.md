...
提交前运行 `go clean -testcache && make check && make integration && make build && make smoke && make bench && git diff --check`。`make smoke` 使用临时二进制和占位密钥，验证 `/readyz`、Bearer 鉴权、OpenAI 风格 `/v1/models` 以及未知模型错误不会回显请求模型，不访问真实 Provider。Docker 可用时再运行 `make image`，不要把真实 Provider Key 写入脚本或 CI。
