...
- 流式响应使用 `text/event-stream`，文本增量转换为 OpenAI SSE，完整结束时输出 `data: [DONE]`；任一 Provider 的上游错误事件或未收到完整结束事件时返回读取错误，不伪造成功结束，也不把上游错误正文放入错误信息。
...
- 不支持的消息角色返回固定错误，不回显客户端提供的角色字符串；未知字段统一返回固定的 `unsupported request field`，公共错误消息不得携带原始用户输入。
...
运行 `make compatibility` 可执行验证本矩阵中的核心边界：支持的 Chat 请求字段、两个输出上限字段的互斥关系、`stream_options.include_usage` 约束、Tools/结构化输出/未知字段拒绝、OpenAI 风格错误 envelope，以及流式响应的 `text/event-stream` 和 `[DONE]` 结束语义。测试只使用进程内 Provider，不访问真实网络。
