...
开发控制面已覆盖 Run 创建、查询、完成、取消和 Request 结算查询；控制变更使用 `Idempotency-Key` 与规范请求哈希。受治理 Chat 在准入后记录本地 Attempt、响应结束后进入结算，已知成本写入唯一账本，未知成本返回 `pending` 并暂停 Run。生产接入前仍需完成租约恢复、跨实例取消和数据库集成测试。
