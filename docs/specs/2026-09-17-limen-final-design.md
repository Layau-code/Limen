...
Gateway Executor 负责请求级总 Context、目标级超时、瞬时错误 Fallback、熔断、连接关闭、普通响应与 SSE 的有界实时转发、客户端取消传播，并按真实调用顺序产生 AttemptReport。Anthropic SSE 上游错误事件或缺少 `message_stop` 时必须传播读取错误，不能写出伪造的 `[DONE]`。Provider 返回错误时 Executor 不消费响应；即使适配器同时返回非空 Body，也必须关闭它。调用前由 Run Coordinator 通过 AttemptStartHook 持久化对应 Attempt，Executor 不持久化 Attempt 或 Ledger；Run Coordinator 根据 ExecutionReport 完成事务。
