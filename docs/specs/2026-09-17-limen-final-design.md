...
首选和备用目标使用同一组硬约束生成，因此备用目标能力不低于原始契约。执行层仍保持：每目标最多一次、仅瞬时错误切换、总 Context 不重置、SSE 成功输出后不切换。普通响应或 SSE 在提交后发生读取/写入中断时只影响结算证据，必须标记为 `partial`/未知，不得把部分响应伪装成完整结算。
...
Gateway Executor 负责请求级总 Context、目标级超时、瞬时错误 Fallback、熔断、连接关闭、普通响应与 SSE 的有界实时转发、客户端取消传播，并按真实调用顺序产生 AttemptReport。OpenAI/Anthropic SSE 上游错误事件或缺少各自终止事件时必须传播读取错误，不能写出伪造的 `[DONE]`。Provider 返回错误时 Executor 不消费响应；即使适配器同时返回非空 Body，也必须关闭它。调用前由 Run Coordinator 通过 AttemptStartHook 持久化对应 Attempt，Executor 不持久化 Attempt 或 Ledger；Run Coordinator 根据 ExecutionReport 完成事务。
...
GET  /v1/limen/keys
POST /v1/limen/keys
POST /v1/limen/keys/{public_prefix}/rotate
...
使用高熵租户级 Key，例如 lmn_live_<public_prefix>_<random_secret>。数据库只保存公开前缀、HMAC-SHA-256 摘要、租户、Scope、状态和有效期；完整 Key 只在创建或轮换时的首次响应显示一次。
...
Explain、Dry Run 和配置版本预演返回标准化契约、配置/算法版本、候选结果、原因码和执行计划，不访问 Provider，也不返回 Prompt、Response、Attempt 或结算正文；真实 Chat 的 Attempt 与结算通过安全响应头、Trailer 和受治理 Request 查询观察。
...
日志使用 Go slog，记录 request_id、run_id、decision_id、逻辑模型、opaque 目标 ID、Attempt、状态、耗时、TTFB、结算状态和原因码。控制面审计额外记录非敏感 `actor_id`，只允许静态标识或 Key 公开前缀。禁止记录 API Key、Authorization、Prompt、Response、Tool 正文、Provider 凭据、上游模型名和原始错误正文。Trace 中的逻辑模型同样必须来自执行计划；兼容模式只记录固定模型前缀，不能直接记录客户端提供的模型字符串。基础 `/metrics` 输出固定计数器和固定桶直方图，目标标签使用稳定 opaque ID，标签有界并要求 `admin` Scope。

Metrics 使用有界 Label：endpoint、状态类别、逻辑模型、目标 ID、结果类别和拒绝原因；不使用 request ID、Run ID、用户 ID 或原始错误作为 Label。请求总耗时和 TTFB 使用固定桶直方图，非法、负数和无穷值不进入指标；固定桶和单指标序列上限共同限制内存与 Prometheus 基数。所有固定指标即使没有样本也输出稳定的 `HELP`/`TYPE` 元数据，避免监控配置依赖请求是否已经发生。
...
- 租约获取失败且没有创建 Attempt 时必须按零成本结算，释放并发名额并保持 Run 可继续准入。
...
- `limen diff --fail-on provider,endpoint` 命中高风险映射变化时返回非零退出码，但输出仍只包含版本、稳定路径、变化类型和 opaque 目标引用。
...
bin/limen diff --base models.current.json --candidate models.next.json --fail-on provider,endpoint
make smoke
make bench
git diff --check
~~~
