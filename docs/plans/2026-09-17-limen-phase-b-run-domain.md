...
> 状态：领域状态机、幂等 MemoryStore、PostgreSQL 迁移/Repository、单机 HTTP Run 控制面、受治理 Chat 准入/结算、请求租约恢复、跨实例取消和未知费用管理员处置已完成；真实 PostgreSQL 多实例故障注入测试和多租户 API Key 管理仍属于后续生产化工作。
